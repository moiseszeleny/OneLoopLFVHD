...
"""
from mpmath import log, polylog, pi, mpf
import mpmath as mp
...
from .roots_mpmath import x1,x2, rootsx2_inv
...
f0np = lambda y:y*log(-1.0*y) - y*log(1.0-y) + log(1.0-1.0/y)- 1.0 

f1np = lambda y: y**2*log(-y) - y**2*log(1-y) - y + log(1-1/y) - mpf('0.5') #lambdify([y],f1,'mpmath')


from .roots_mpmath import y11 as y11np
from .roots_mpmath import y12 as y12np
from .roots_mpmath import y21 as y21np
from .roots_mpmath import y22 as y22np
...
#@jit
def b2_0np(mj,M0,M2):
    return -log(M2**2)-f02sum(mj,M0,M2)

#@jit
def b1_1np(mi,M0,M1):
    return -(mpf('0.5')*(-log(M1**2))-f01sum(mi,M0,M1)+ mpf('0.5')*f11sum(mi,M0,M1))

#@jit
def b2_1np(mj,M0,M2): 
    return mpf('0.5')*(-log(M2**2))-f02sum(mj,M0,M2) + mpf('0.5')*f12sum(mj,M0,M2)
