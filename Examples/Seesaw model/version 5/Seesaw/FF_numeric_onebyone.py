...
from Unu_seesaw import diagonalizationMnu#_svd as diagonalizationMnu
# %%
...
def numeric_sum_diagramsOne(a, b, quirality='L'):
    ...
    for Set in [TrianglesOneFermion, Bubbles]:  # TrianglesOneFermion,Bubbles
        for dia in Set:
            ...
            f = lambdify(
                ...
                modules=[
                    pave_functions(valores[mh], a, b, lib='mpmathDelta'),
                    'mpmath'
                    ]
                )  # numeric Form factor
            # print(f(1,2,3))
    ...
    def sum_over_index_i(m6):
        ...
        for FFab in FFOneFer_list:
            FF_sum_over_i = sumOne(m6, FFab, a, b)
...
# %%
# pdb.set_trace()
def sumatwo(m6, k, a, b, quirality='L'):
    xs = []
    if quirality == 'L':
        g = fL(k, a, b)
    elif quirality == 'R':
...
def totaltwo(m6, a, b, quirality='L'):
    return np.array(
        [
            sumatwo(m6, 0, a, b, quirality),
            sumatwo(m6, 1, a, b, quirality)
            ]
        )
...
if __name__ == "__main__":
    ...
    # %%
    FF_frame23L.to_csv(
        'data_frames/FF_frame_23L.txt',
        sep='\t', index=None)
    FF_frame23R.to_csv(
        'data_frames/FF_frame_23R.txt',
        sep='\t', index=None)

    FF_frame13L.to_csv(
        'data_frames/FF_frame_13L.txt',
        sep='\t', index=None)
    FF_frame13R.to_csv(
        'data_frames/FF_frame_13R.txt',
        sep='\t', index=None)

    FF_frame12L.to_csv(
        'data_frames/FF_frame_12L.txt',
        sep='\t', index=None)
    FF_frame12R.to_csv(
        'data_frames/FF_frame_12R.txt',
        sep='\t', index=None)
    print('Finalizado')
