...
#print(Upmns)
Upmns_np = np.array([[float(Upmns[r, s]) for r in range(3)] for s in range(3)]).T
#print(Upmns_np)

Um = Upmns_np.dot(mndsqrt(m1, m2, m3))#Upmns_np
#Um = Upmns_np.dot(Upmns_np)
print(type(mndsqrt(m1, m2, m3)))
...
def cota_perturvativity(mD, v):
    return np.abs(mD)**2 < 3*np.pi*v**2
...
print(type(np.abs(mD_CI(1e4, 1e-7, a-1, b-1))**2))
plt.loglog(tb, 3*np.pi*v2(tb)**2)
plt.fill_between(tb, 3*np.pi*v2(tb)**2, alpha=0.5,label='Permitido')
...
plt.xlabel(r'$\tan{\beta}$', fontsize=25)
...
plt.ylabel(r'Cotas de perturvatividad', fontsize=s)
plt.tick_params(labelsize=s)
plt.legend(fontsize=s)
plt.xlim(1e-3, 1e3)
plt.legend(frameon=False, ncol=2, fontsize=18, loc=9)
