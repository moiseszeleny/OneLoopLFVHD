...
#######################################################3
# Total sum over diagramas with one fermion
###########################################################
...
def numeric_sum_diagramsOne(ms,a,b,mHpm, mA, alpha, beta, l5,
    ...
    if type_2HDM == 'I':
        Yuk_common = typeI_h(alpha,beta)
    elif type_2HDM == 'II':
        Yuk_common = typeII_h(alpha,beta)
    elif type_2HDM == 'Lepton-specific'.upper():
        Yuk_common = Lepton_specific_h(alpha,beta)
    elif type_2HDM == 'Flipped'.upper():
        Yuk_common = Flipped_h(alpha,beta)
    else:
...
################################################
modelo_typeI = lambda caso: {'modelo':'I','carpeta':'Type_I',
                             'filename':f"TypeI_Cab095_caso{caso['n']}"}
modelo_typeII = lambda caso: {'modelo':'II','carpeta':'Type_II',
                             'filename':f"TypeII_Cab095_caso{caso['n']}"}
modelo_lepton = lambda caso: {'modelo':'Lepton-specific','carpeta':'Lepton_specific',
                              'filename':f"Lepton_specific_Cab095_caso{caso['n']}"}
modelo_flipped = lambda caso: {'modelo':'Flipped','carpeta':'Flipped',
        ...
        for caso in [Benchmarck1]:#casos
            ...
            def Γhl2l3(tb):
                AL = ALtot23_caso1(tb,m6=m6_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           H_a='h',type_2HDM=modelo(caso)['modelo'])
