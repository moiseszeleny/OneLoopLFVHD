...
if __name__ == '__main__':    
    ...
    for caso in casos1:
        ...
        path = f"output/{modelo_typeI(caso)['carpeta']}/"
        plt.savefig(path+f"{modelo_typeI(caso)['filename']}.png",dpi=100)
        #plt.show()
        ...
        df.to_csv(path+f"{modelo_typeI(caso)['filename']}.txt",sep='\t')
        print(path+f"{modelo_typeI(caso)['filename']}.txt has been completed")
    plt.show()
