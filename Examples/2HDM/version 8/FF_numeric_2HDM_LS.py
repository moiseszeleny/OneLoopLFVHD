...
Benchmarck2 = {'mA':'300','mHpm':'500.0','l5':'1', 'cab':'0.0', 'n':'2'}
...
casos1 = [Benchmarck1, Benchmarck2]
...
if __name__ == '__main__':    
    ...
    for caso in casos1:
        ...
        path = f"output/{modelo_lepton(caso)['carpeta']}/"
        plt.savefig(path+f"{modelo_lepton(caso)['filename']}.png",dpi=100)
        #plt.show()
        ...
        df.to_csv(path+f"{modelo_lepton(caso)['filename']}.txt",sep='\t')
        print(path+f"{modelo_lepton(caso)['filename']}.txt has been completed")
    plt.show()
