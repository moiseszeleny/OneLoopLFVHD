...
from modelos_2HDM import coeff_typeII_h as coeff_h
...
if __name__ == '__main__':    
    ...
    for caso in casos1:
        ...
        path = f"output/{modelo_typeII(caso)['carpeta']}/"
        plt.savefig(path+f"{modelo_typeII(caso)['filename']}.png",dpi=100)
        #plt.show()
        ...
        df.to_csv(path+f"{modelo_typeII(caso)['filename']}.txt",sep='\t')
        print(path+f"{modelo_typeII(caso)['filename']}.txt has been completed")
    plt.show()
