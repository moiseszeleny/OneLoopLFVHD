...
def numeric_sum_diagrams(ms,a,b,mHpm, mA, cab, l5,
    ...
    def FFOne(tb):
        ...
        out = sum_diagrams(a,b,ms,mHpm,betaf(tb),l5,
...
###############################################
Benchmarck1 = {'mA':'800','mHpm':'1000.0','l5':'0.1', 'cab':'0.01', 'n':'1'}
Benchmarck2 = {'mA':'800','mHpm':'1000.0','l5':'1', 'cab':'0.01', 'n':'2'}
...
def ALtot23_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ALtot23(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)
def ARtot23_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ARtot23(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)

def ALtot13_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ALtot13(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)
def ARtot13_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ARtot13(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)

def ALtot12_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ALtot12(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)
def ARtot12_caso1(tb, cab,mHpm,mA, l5,type_2HDM=typeI_h):
    return ARtot12(mh,mHpm, mA, tb, cab, l5,type_2HDM=type_2HDM)
...
if __name__ == '__main__':
    ...
    for modelo in modelos:#modelos:
        for caso in casos:
            ...
            # tb, cab,mHpm,mA, l5
            def Γhl2l3(tb):
                AL = ALtot23_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
                AR = ARtot23_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
                return Γhlilj(AL,AR, mh,ml[2],ml[3])
            
            def Γhl1l3(tb):
                AL = ALtot13_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
                AR = ARtot13_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
                return Γhlilj(AL,AR, mh,ml[1],ml[3])
            
            def Γhl1l2(tb):
                AL = ALtot12_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
                AR = ARtot12_caso1(tb,cab=cab_val,mHpm=mHpm_val,mA=mA_val,l5=l5_val,
                                           type_2HDM=modelo(caso)['modelo'])
            ...
            plt.title(f"$m_A=${caso['mA']} GeV"+ r" $m_{H^{\pm}}=$" + f"{caso['mHpm']} GeV" + r" $\cos(\beta - \alpha)=$" + f"{caso['cab']}" + r" $\lambda_5=$" + f"{caso['l5']}" ,fontsize=18)
